...
ostream & operator<<(ostream & os, const spectra & s)
{
	os << s.file_name << "	" << s.scan_no << "	" << s.exp_mh << "	" << s.charge << "	" << s.q_value << "	" << s.seq << "	"
		...
		<< s.others;
	//当处理理论突变结果时，启用暗绿语句
	os << "	" << s.marker << "	" << s.is_mut << "	" << s.ratio << '\t' << s.intensity1 << '\t' << s.intensity2;//由于理论突变文件中，突变信息隐含在ENSP号末尾的字段（_SAP_),且取ensp号时会切掉此字段，为在输出文件中区分突变链同正常链，故在最后一列输出此项
	return os;
}
...
istream & operator>>(istream & in, vector<spectra> & list_result)
{
	...
	while (in >> temp.file_name) {
		...
		//当处理openresearch结果时，启用暗绿语句
		///in >> temp.mut_count;
		if(temp.q_value<0.01&&temp.targe=="target"&&temp.is_mut==true&&temp.prot.length()<=22)
